  ...
  return (
    <main style={{ background: "#FFF8EF", minHeight: "100vh" }}>
      ...
      <div style={{ maxWidth: "960px", margin: "0 auto", padding: "1.5rem" }}>
        ...
        {!loading && locales.length > 0 && (
          <>
            <div style={{ display: "grid", gap: "1rem", gridTemplateColumns: "repeat(auto-fill, minmax(260px,1fr))" }}>
              {locales.map(local => (
                <a key={local.id} href={local.slug ? `/locales/${slug}/${local.slug}` : undefined} style={{
        ...
        {/* Newsletter de la ciudad.
            El otro formulario (más arriba) sólo aparece cuando la ciudad está VACÍA,
            así que justo las páginas con contenido —las que reciben el tráfico— no
            pedían el email a nadie. Este bloque cubre ese hueco: mismo estilo, copy
            distinto (planes cada semana, no "te avisamos cuando esté lista") y sin
            redirigir al inicio al terminar. */}
        {!noData && totalCiudad !== null && totalCiudad > 0 && (
            ...
            {subscribeStatus === "ok" ? (
              <>
                ...
                <p style={{ color: "#15803D", fontSize: "0.9rem" }}>
                  Te escribiremos con los planes de {nombreCiudad}.
                </p>
              </>
            ) : (
              <>
                ...
                <h2 style={{ fontWeight: 800, color: "#1C1917", fontSize: "1.15rem", marginBottom: "0.4rem", letterSpacing: "-0.02em" }}>
                  Los mejores planes de {nombreCiudad}, cada semana
                </h2>
                <p style={{ color: "#78716C", maxWidth: "420px", margin: "0 auto 1.25rem", lineHeight: 1.6, fontSize: "0.92rem" }}>
                  Terrazas nuevas, eventos y sitios que merecen la pena. Un correo, sin relleno.
                </p>
                <div ref={containerRef} />
                <form
                  onSubmit={async e => {
                    ...
                    try {
                      ...
                      const res = await fetch("/api/subscribe", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({ email, ciudad: nombreCiudad, cf_token }),
                      });
                ...
                >
                  <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
                    ...
                    <button type="submit" disabled={subscribeStatus === "loading"} style={{
                      ...
                      {subscribeStatus === "loading" ? "..." : "Apuntarme →"}
                    </button>
                  </div>
                  {subscribeStatus === "error" && (
                    <p style={{ color: "#DC2626", fontSize: "0.85rem", marginTop: "0.5rem" }}>
