...
type Step = "buscar" | "nuevo_local" | "confirmar";
...
export default function AddLocal() {
  ...
  useEffect(() => {
    if (!localStorage.getItem("local_token")) router.push("/local/login");
  }, [router]);
  ...
  function seleccionarLocal(r: LocalResult) {
    if (r.claimed) { setError("Este local ya tiene una cuenta registrada"); return; }
  ...
  async function handleConfirmar(e: React.FormEvent) {
    ...
    const payload = esNuevo
      ? { action: "add_local", nombre: nuevoNombre, tipo: nuevoTipo, ciudad: nuevaCiudad, direccion: nuevaDireccion || undefined, telefono: nuevoTelefono || undefined, web: nuevoWeb || undefined, instagram: nuevoInstagram || undefined }
      : { action: "add_local", local_id: localSeleccionado.id };
    ...
    if (!res.ok) {
      setError(data.error || "Error al añadir el local");
      setEnviando(false);
      return;
    }

    // Cambiar al nuevo local y volver al dashboard
  ...
  return (
    <main style={{ background: "#FFF8EF", minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", padding: "2rem 1rem" }}>
      ...
      <div style={{ width: "100%", maxWidth: "520px", display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.5rem" }}>
        ...
        <Link href="/local/dashboard" style={{ fontSize: "0.82rem", color: "#A8A29E", textDecoration: "none" }}>
          ← Volver al dashboard
        </Link>
      </div>
      ...
      {step === "buscar" && (
        <div style={cardStyle}>
          <h1 style={{ fontWeight: 800, fontSize: "1.3rem", color: "#1C1917", marginBottom: "0.35rem" }}>Añadir otro local</h1>
          <p style={{ fontSize: "0.85rem", color: "#78716C", marginBottom: "1.5rem" }}>
            Busca el local que quieres gestionar y añádelo a tu cuenta.
          </p>
      ...
      {step === "nuevo_local" && (
        <div style={cardStyle}>
          ...
          <h2 style={{ fontWeight: 800, fontSize: "1.2rem", color: "#1C1917", marginBottom: "0.35rem" }}>Datos del nuevo local</h2>
      ...
      {/* STEP: CONFIRMAR */}
      {step === "confirmar" && localSeleccionado && (
        <div style={cardStyle}>
          <button onClick={() => { setStep(localSeleccionado.id === "__nuevo__" ? "nuevo_local" : "buscar"); setLocalSeleccionado(null); setError(""); }} style={{ background: "none", border: "none", cursor: "pointer", color: "#A8A29E", fontSize: "0.85rem", marginBottom: "1rem", padding: 0 }}>
            ← Volver
          </button>
          ...
          <h2 style={{ fontWeight: 800, fontSize: "1.2rem", color: "#1C1917", marginBottom: "0.35rem" }}>Confirmar</h2>
          <p style={{ fontSize: "0.85rem", color: "#78716C", marginBottom: "1.5rem" }}>
            Este local se añadirá a tu cuenta. Podrás gestionarlo desde el panel.
          </p>

          <form onSubmit={handleConfirmar} style={{ display: "flex", flexDirection: "column", gap: "0.85rem" }}>
            {error && <p style={{ color: "#EF4444", fontSize: "0.85rem" }}>{error}</p>}
            <button type="submit" disabled={enviando} style={{
              padding: "0.9rem", borderRadius: "0.875rem", border: "none", cursor: "pointer",
              background: "linear-gradient(135deg,#FB923C,#F59E0B)", color: "white", fontWeight: 800, fontSize: "1rem",
            }}>
              {enviando ? "Añadiendo..." : "Añadir local a mi cuenta →"}
            </button>
          </form>
        </div>
      )}
